...
def proyectarPts(T, wz):
    ...
    xy = np.zeros((T.shape[0], wz.shape[1]))
    filas_xy=range(T.shape[0])
    columnas_xy = range(wz.shape[1])
    columnas_filas_T_wz= range(wz.shape[0]) 
    for fila in filas_xy: 
        for cols in columnas_xy: 
            for i in columnas_filas_T_wz:
                xy[fila][cols]+=T[fila][i]*wz[i][cols]
